        ...
        <transition  name="fade">
            <div class="home" :style="{backgroundImage:'url('+senceImgs+')'}" v-cloak v-show="showScene">
                ...
                <div class="change_scence">
                    ...
                    <button class="start_Animation" @click="startAnimation">开始</button>
...
import { Huginn } from '../api/collections'
...
export default {
    ...
    mounted(){
        ...
        setTimeout(()=>{
            var str = this.getData[0].message;
            var indexx = str.indexOf('雨',0);
            if(indexx<0){
                this.sunBtn()
            }else{
                this.rainBtn()
    ...
    methods:{
        ...
        startAnimation(){
            //点击开始 调用接口 post 传相应的 应用参数 获取当前天气数据 执行 相应的动画
        },
        sunBtn(){
            clearTimeout(this.timerRain.rain1);
            clearTimeout(this.timerRain.rain2);
